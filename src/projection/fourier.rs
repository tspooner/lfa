...
mod tests {
    ...
    fn test_symmetry() {
        ...
        assert_eq!(
            f.project_expanded(&[-1.0]),
            f.project_expanded(&[1.0])
        );
        assert_eq!(
            f.project_expanded(&[-0.5]),
            f.project_expanded(&[0.5])
        );
    }
    ...
    fn test_order1_1d() {
        ...
        assert!(vec_compare(
            &f.project_expanded(&vec![-1.0]).as_slice().unwrap(),
            &[-1.0],
            1e-6
        ));
        assert!(vec_compare(
            &f.project_expanded(&vec![-0.5]).as_slice().unwrap(),
            &[0.0],
            1e-6
        ));
        assert!(vec_compare(
            &f.project_expanded(&vec![0.0]).as_slice().unwrap(),
            &[1.0],
            1e-6
        ));
        assert!(vec_compare(
        ...
        assert!(vec_compare(
            &f.project_expanded(&vec![-2.0 / 3.0]).as_slice().unwrap(),
            &[-1.0],
            1e-6
        ));
        assert!(vec_compare(
            &f.project_expanded(&vec![-1.0 / 3.0]).as_slice().unwrap(),
            &[1.0],
            1e-6
        ));
        assert!(vec_compare(
    ...
    fn test_order2_1d() {
        ...
        assert_eq!(
            f2.project_expanded(&[-1.0]),
            f1.project_expanded(&[-1.0])
        );
        assert_eq!(
            f2.project_expanded(&[-0.5]),
            f1.project_expanded(&[-0.5])
        );
        assert_eq!(
            f2.project_expanded(&[0.0]),
            f1.project_expanded(&[0.0])
        );
        assert_eq!(
            f2.project_expanded(&[0.5]),
            f1.project_expanded(&[0.5])
        ...
        assert_eq!(
            f2.project_expanded(&[-2.0 / 3.0]),
            f1.project_expanded(&[-2.0 / 3.0])
        );
        assert_eq!(
            f2.project_expanded(&[-1.0 / 3.0]),
            f1.project_expanded(&[-1.0 / 3.0])
        );
        assert_eq!(
            f2.project_expanded(&[1.0 / 3.0]),
            f1.project_expanded(&[1.0 / 3.0])
    ...
    fn test_order1_2d() {
        ...
        assert!(vec_compare(
            &f.project_expanded(&[0.0, 5.0]).as_slice().unwrap(),
            &[1.0 / 3.0; 3],
            1e-6
        ));
        assert!(vec_compare(
            &f.project_expanded(&[0.5, 5.0]).as_slice().unwrap(),
            &[4.24042024e-17, 3.07486821e-1, 6.92513179e-1],
            1e-6
        ));
        assert!(vec_compare(
            &f.project_expanded(&[0.0, 5.5]).as_slice().unwrap(),
            &[6.92513179e-1, 3.07486821e-1, 4.24042024e-17],
            1e-6
        ));
        assert!(vec_compare(
            &f.project_expanded(&[0.5, 5.5]).as_slice().unwrap(),
            &[1.01093534e-16, -1.0, 1.01093534e-16],
            1e-6
        ));
        assert!(vec_compare(
            &f.project_expanded(&[1.0, 5.5]).as_slice().unwrap(),
            &[-5.04567213e-1, -4.95432787e-1, 3.08958311e-17],
            1e-6
        ));
        assert!(vec_compare(
            &f.project_expanded(&[0.5, 6.0]).as_slice().unwrap(),
            &[3.08958311e-17, -4.95432787e-1, -5.04567213e-1],
            1e-6
        ));
        assert!(vec_compare(
            &f.project_expanded(&[1.0, 6.0]).as_slice().unwrap(),
            &[-0.44125654, -0.11748691, -0.44125654],
            1e-6
        ));
    }
